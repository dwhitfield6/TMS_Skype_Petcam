...
void InitADC(void)
{
	/* Set ADC ISRs */
	SYS_Unlock();
	PieVectTable.ADCA1_INT = &ISR_ADC_AUDIO;
	SYS_Lock();

	SYS_EnableInterruptGroup(INTERRUPT_GROUP1);	// Group for ADC
	...
	ADC_SetupContinuousA(ADC_AUDIO_2_GPIO);
}
...
void ADC_SetupContinuousA(unsigned short channel)
{
	...
	AdcaRegs.ADCINTSEL1N2.bit.INT1E = 0;		// 0 ADCINT3 is disabled
	AdcaRegs.ADCINTSEL1N2.bit.INT2E = 0; 		// 0 ADCINT3 is disabled
	AdcaRegs.ADCINTSEL3N4.bit.INT3E = 0; 		// 0 ADCINT3 is disabled
	AdcaRegs.ADCINTSEL3N4.bit.INT4E = 0; 		// 0 ADCINT3 is disabled
...
void ADC_InterruptA(unsigned char state)
{
	...
	if(state)
	{
		PieCtrlRegs.PIEIER1.bit.INTx1  = 1;		// PIE Group 1, INT1
		AdcaRegs.ADCINTSEL1N2.bit.INT1E = 1;	// enable INT1 flag
	}
	else
	{
		PieCtrlRegs.PIEIER1.bit.INTx1  = 0;		// PIE Group 1, INT1
...
{
	if(AdcaRegs.ADCSOCFLG1.bit.SOC0)
	{
		/*  Trigger has been received and sample is pending for SOC0 */
		return TRUE;
	}
	/* No sample pending for SOC0 */
	return FALSE;
}
