...
// Configure GPIO pins as SDFM pins
	...
	//All the filter modules are asynchronized when master filter bit is enabled before individual filter modules are enabled.
...
void InitEPwm(void)
{
...
#endif
    EALLOW;
	CpuSysRegs.PCLKCR0.bit.TBCLKSYNC = 1;    //Allows all users to globally synchronize all enabled ePWM modules to the time-base clock (TBCLK)

	// Setup TBCLK

    (*EPWM[gPWM_number]).TBPHS.bit.TBPHS = 0x0000;          // Phase is 0
    (*EPWM[gPWM_number]).TBCTR = 0x0000;                     // Clear counter
    (*EPWM[gPWM_number]).TBPRD = EPWM_TIMER_TBPRD;          // Set timer period 801 TBCLKs
    ...
    (*EPWM[gPWM_number]).TBCTL.bit.HSPCLKDIV = TB_DIV1;      // Clock ratio to SYSCLKOUT
    ...
    //(*EPWM[PWM_number]).AQCTLA.bit.PRD = 1;

    // Set actions
    (*EPWM[gPWM_number]).AQCTLB.bit.CBU = 3;            // Set PWM1A on event A, up
                                                  // count

    //(*EPWM[PWM_number]).AQCTLB.bit.PRD = 1;
