...
static inline void setFreq(void)
{
    ...
    sgen.freq = ((float)outputFreq_hz/maxOutputFreq_hz)*0x8000; // Range(Q15) = 0x8000 -> 0x7FFF, freq(Q15) = (Required_Freq_hz/Max_Freq_hz)*0x8000
...
interrupt void cpu_timer0_isr(void)
{
	...
	DAC_PTR[DAC_NUM]->DACVALS.all = sgen_out; // Write current ramp value to buffered DAC
