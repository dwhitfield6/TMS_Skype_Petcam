...
//*****************************************************************************
//
// This function performs a walking 0 & 1 For SDRAM RD & WR
//
//*****************************************************************************
char
sdram_data_walk(Uint32 start_addr, Uint32 mem_size)
{
	...
	for (i=0; i < mem_size; i=i+64)
	{
        for (m=0; m < 2; m++)
        {
            ...
            for (k=0; k < 32; k++)
            {
      	        if(m==0)   __addr32_write_uint32(XMEM_p, sdram_wdl);
      	        else       __addr32_write_uint32(XMEM_p, ~sdram_wdl);
      	        XMEM_p = XMEM_p+2;
...
//*****************************************************************************
//
// This function performs a toggle on each address bit. In this case
// memory assumed is 4Mb.
...
//*****************************************************************************
//
// This function performs different data type (HALFWORD/WORD) access.
//
//*****************************************************************************
char
...
void main(void)
{
	...
	//PAGESIZE=0 (256 elements per ROW), IBANK = 2 (4 BANK), CL = 3, NM = 0 (32bit)
