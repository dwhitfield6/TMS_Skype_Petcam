...
static inline void setFreq(void)
{
    sgen.step_max = (maxOutputFreq_hz*0x10000*4)/samplingFreq_hz; // Range(Q0) = 0x0000 -> 0x7FFF, step_max(Q0) = (Max_Freq_hz*0x10000*4)/Sampling_Freq_hz
...
interrupt void cpu_timer0_isr(void)
{
	...
	DAC_PTR[DAC_NUM]->DACVALS.all = sgen_out; // Write current square value to buffered DAC
