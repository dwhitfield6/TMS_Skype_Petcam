...
//
//Defines for the command codes sent to the kernel.
//
...
Uint16 SCI_SendPacket(Uint16 command, Uint16 status, Uint16 length, Uint16* data);
...
// Uint32 SCI_GetFunction(Uint32 BootMode)
//--------------------------------------------
//This function first initializes SCIA and performs
...
Uint32 SCI_GetFunction(Uint32  BootMode)
{
    ...
    while( command != RESET_CPU1)
    {
        ...
        {
        	Uint32 sectors = (Uint32)( ((Uint32)data[1] << 16) | (Uint32)data[0]);
    	...
    	while( SCI_SendPacket(command, statusCode.status, 6, (Uint16*)&statusCode.address) ){}
    	//get next Packet
    	command = SCI_GetPacket(&length, data); //get next packet

    }//end while( command != RESET_CPU1)
    //RESET_CPU1
...
//#################################################
// void SCI_SendPacket(Uint16, Uint16, Uint16, Uint16*)
//----------------------------------------------
// Sends a Packet to the host which contains
...
//#################################################
// void SCIA_SendChecksum(void)
//----------------------------------------------
// Sends the Global checksum value
//----------------------------------------------
...
//#################################################
// void SCIA_SendWord(Uint16 word)
//----------------------------------------------
// Sends a Uint16 word.
//----------------------------------------------
void SCI_SendWord(Uint16 word)
...
//#################################################
// Uint16 SCI_GetPacket(Uint16* data)
//-----------------------------------------------
// This routine receives the packet, returns the
