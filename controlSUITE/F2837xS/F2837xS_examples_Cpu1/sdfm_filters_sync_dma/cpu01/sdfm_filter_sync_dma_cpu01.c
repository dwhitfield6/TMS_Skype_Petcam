...
// Enable CPU INT7 interrupts are enabled
	...
	Sdfm_configureData_filter(gPeripheralNumber, FILTER2, FILTER_ENABLE, SINC3, OSR_256, DATA_16_BIT, SHIFT_9_BITS);
	Sdfm_configureData_filter(gPeripheralNumber, FILTER3, FILTER_ENABLE, SINC3, OSR_256, DATA_16_BIT, SHIFT_9_BITS);
	Sdfm_configureData_filter(gPeripheralNumber, FILTER4, FILTER_ENABLE, SINC3, OSR_256, DATA_16_BIT, SHIFT_9_BITS);

	//Enable Master filter bit: Unless this bit is set none of the filter modules can be enabled.
	//All the filter modules are synchronized when master filter bit is enabled after individual filter modules are enabled.
	...
	Sdfm_configureInterrupt(gPeripheralNumber, FILTER1, IEH_DISABLE, IEL_DISABLE, MFIE_ENABLE, AE_ENABLE);
	Sdfm_configureInterrupt(gPeripheralNumber, FILTER2, IEH_DISABLE, IEL_DISABLE, MFIE_ENABLE, AE_ENABLE);
	Sdfm_configureInterrupt(gPeripheralNumber, FILTER3, IEH_DISABLE, IEL_DISABLE, MFIE_ENABLE, AE_ENABLE);
	Sdfm_configureInterrupt(gPeripheralNumber, FILTER4, IEH_DISABLE, IEL_DISABLE, MFIE_ENABLE, AE_ENABLE);

	//  Enable master interrupt so that any of the filter interrupts can trigger by SDFM interrupt to CPU
...
void DMA_setup()
{
	...
	DMA1Source = (Uint16 *)(0x5E17);DMA1Dest = &Filter1_Result[0];
	DMA2Source = (Uint16 *)(0x5E27);DMA2Dest = &Filter2_Result[0];
	DMA3Source = (Uint16 *)(0x5E37);DMA3Dest = &Filter3_Result[0];
	DMA4Source = (Uint16 *)(0x5E47);DMA4Dest = &Filter4_Result[0];
    ...
    DMACH2BurstConfig(ONE_WORD_BURST,SOURCE_NO_ADDRESS_CHANGE,DESTINATION_INCREMENT_ONE_ADDRESS);             					   // BURST size = 1 | Source step size = 0 | Dest step size += 1
    DMACH3BurstConfig(ONE_WORD_BURST,SOURCE_NO_ADDRESS_CHANGE,DESTINATION_INCREMENT_ONE_ADDRESS);             					   // BURST size = 1 | Source step size = 0 | Dest step size += 1
    DMACH4BurstConfig(ONE_WORD_BURST,SOURCE_NO_ADDRESS_CHANGE,DESTINATION_INCREMENT_ONE_ADDRESS);             					   // BURST size = 1 | Source step size = 0 | Dest step size += 1
    ...
    DMACH2TransferConfig(TRANSFER_SIZE_0x800,TRANSFER_STEP_SOURCE_NO_CHANGE,TRANSFER_STEP_DEST_INCREMENT_ONE_ADDRESS);            // Transfer size = 0x1000 | Source step size = 0 | Dest step size += 1
    DMACH3TransferConfig(TRANSFER_SIZE_0x800,TRANSFER_STEP_SOURCE_NO_CHANGE,TRANSFER_STEP_DEST_INCREMENT_ONE_ADDRESS);            // Transfer size = 0x1000 | Source step size = 0 | Dest step size += 1
    DMACH4TransferConfig(TRANSFER_SIZE_0x800,TRANSFER_STEP_SOURCE_NO_CHANGE,TRANSFER_STEP_DEST_INCREMENT_ONE_ADDRESS);            // Transfer size = 0x1000 | Source step size = 0 | Dest step size += 1
...
void DMA_configure(void)
{
    ...
    DMACH1ModeConfig(DMA_SD1FLT1,PERINT_ENABLE,ONESHOT_DISABLE,CONT_DISABLE,SYNC_DISABLE,SYNC_SRC,OVRFLOW_DISABLE,SIXTEEN_BIT,CHINT_END,CHINT_ENABLE);
    DMACH2ModeConfig(DMA_SD1FLT2,PERINT_ENABLE,ONESHOT_DISABLE,CONT_DISABLE,SYNC_DISABLE,SYNC_SRC,OVRFLOW_DISABLE,SIXTEEN_BIT,CHINT_END,CHINT_ENABLE);
    DMACH3ModeConfig(DMA_SD1FLT3,PERINT_ENABLE,ONESHOT_DISABLE,CONT_DISABLE,SYNC_DISABLE,SYNC_SRC,OVRFLOW_DISABLE,SIXTEEN_BIT,CHINT_END,CHINT_ENABLE);
    DMACH4ModeConfig(DMA_SD1FLT4,PERINT_ENABLE,ONESHOT_DISABLE,CONT_DISABLE,SYNC_DISABLE,SYNC_SRC,OVRFLOW_DISABLE,SIXTEEN_BIT,CHINT_END,CHINT_ENABLE);
