...
// This function can restore the GPIOs to their previous state,
...
#if RETAINM0M1
// Fill both memory sections with pseudo-random data
void FillMem()
{
	Uint32 temp, seed, c;
...
// Clear the memory to a known state
void ClearMem()
{
	Uint32 c;
	for (c = 0; c < sizeof(hibstate); c++)
...
//Verify that the saved M0 and M1 RAM data is intact
Uint16 VerifyMem()
{
	Uint32 temp, seed;
	Uint16 c;
